...
int TrajectoryGenerator::BezierPloyCoeffGeneration(
            const vector<Cube> &corridor, const MatrixXd &MQM,
            const MatrixXd &pos, const MatrixXd &vel, const MatrixXd &acc,
            const double maxVel, const double maxAcc,
            const int traj_order, const double minimize_order,
            const double margin,
...
//######################################################################
    ...
    //  设置第j个变量的边界值 { for j=1, ...,ctrlP_num : blx[i] <= var[j] <= bux[i] }
    ...
    //  设置第i个约束边界值 { for i=1, ...,con_num : blc[i] <= constraint i <= buc[i] }
...
quadrotor_msgs::PolynomialTrajectory TrajectoryGenerator::getBezierTraj(const MatrixXd & polyCoeff, const VectorXd & _seg_time, 
                                                                int _traj_order, int _seg_num, int _traj_id, ros::Time t_now )
{
    ...
    //   initialVel = getVelFromBezier(polyCoeff, _Cv, _traj_order, 0, 0);
    //   finalVel = getVelFromBezier(polyCoeff, _Cv, _traj_order, traj.time[traj.num_segment - 1] ,traj.num_segment - 1); 
...
Vector3d TrajectoryGenerator::getVelFromBezier(const MatrixXd & polyCoeff, const VectorXd & _Cv, int _traj_order, double t_now, int seg_now) {
  Vector3d ret = VectorXd::Zero(3);
  VectorXd ctrl_now = polyCoeff.row(seg_now);
  ...
  for (int i = 0; i < 3; i++) 
    for (int j = 0; j < ctrl_num1D; j++)
        ret(i) += _Cv(j) * _traj_order * ( ctrl_now(i * ctrl_num1D + j + 1) - ctrl_now(i * ctrl_num1D + j))
                      * pow(t_now, j) * pow((1 - t_now), (_traj_order - j - 1) ); 
    
  return ret;
}
VectorXd TrajectoryGenerator::getStateFromBezier(const MatrixXd & polyCoeff, const VectorXd & _C, const VectorXd & _Cv, const VectorXd & _Ca, const VectorXd & _Cj, int _traj_order, double t_now, int seg_now )
{
